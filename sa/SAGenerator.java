...
//	HashMap<Integer, ArrayList<Integer>> adjs;	
	...
	// data structure
	ArrayList<TBucketSA> tBucketList;
	...
	long genTime = 0;
	long initTime = 0;
	public SAGenerator(int numNodesFinal, String dType, double dExp, String tType, double tExp, String fType, double fExp, int fMax, int initAttract)
	{
		long t_init_0 = System.nanoTime();
		
		this.numNodesFinal = numNodesFinal;
		this.dType = dType;
		System.out.println("fixing dExp = 1");
		this.dExp = 1.0;
		this.tType = tType; 
		this.tExp = tExp;
		this.fType = fType;
...
//			adjs.put(i, new ArrayList<Integer>());
	...
	{
		tBucketList = new ArrayList<TBucketSA>(2 * (int)(Math.log(this.numNodesFinal) / Math.log(2) + 1)); // upper bound
		sumTBucketWeight = 0;
		...
		TBucketSA tBucket0 = new TBucketSA(0, this);
		tBucketList.add(0, tBucket0);
		TBucketSA tBucket1 = new TBucketSA(1, this);
	...
	private void mergeTBuckets()
	{
		...
		{
			TBucketSA currentBucket = tBucketList.get(index);
			TBucketSA nextBucket = tBucketList.get(index + 1);
			TBucketSA nextNextBucket = tBucketList.get(index + 2);
	...
	private void addEdges(int numNodesCurrent)
	{
		...
		while(true)
		{
			...
			//{
				...
				fromBucket = (TBucketSA)(returnList.get(1));
				toNode = (Integer)(returnList.get(2));
				toBucket = (TBucketSA)(returnList.get(3));
			...
			else
			{
				...
				{
					toBucket.addNodeDegreeByOne(toNode);
					TBucketSA tBucketNew = new TBucketSA(fromNode, this);
				...
				{
					fromBucket.addNodeDegreeByOne(fromNode);
					TBucketSA tBucketNew = new TBucketSA(toNode, this);
					tBucketList.add(0, tBucketNew);
