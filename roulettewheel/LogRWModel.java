...
//		HashMap<Integer, ArrayList<Integer>> adjs;	
	...
	long genTime = 0; 
	long initTime = 0;
	public LogRWModel(int numNodesFinal, String dType, double dExp, String tType, double tExp, String fType, double fExp, int fMax, int initAttract)
	{
...
//					adjs.get(j).add(i);
	...
	private double tpaScore(int nodeId, int numNodesCurrent)
	{
		if(nodeId < numNodesCurrent){
			...
			tpa_score *= Math.pow(Math.log(Math.max(deg[nodeId], 1)), dExp);
